...
// 빗썸 아이콘 URL 패턴 (MD5 해시 사용)
const BITHUMB_ICON_URL_PATTERN =
  "https://content.bithumb.com/resources/img/coin/coin-{hash}.png";
...
function downloadAndResizeIcon(ticker) {
  return new Promise((resolve, reject) => {
    const hash = getMD5Hash(ticker);
    const url = BITHUMB_ICON_URL_PATTERN.replace("{hash}", hash);
    const dest = `icons/bithumb/${ticker}.webp`;
    ...
    console.log(`📥 다운로드 및 리사이징 중: ${ticker} (MD5: ${hash})`);
...
async function downloadAllIcons(tickers, delay = 100) {
  ...
  console.log(`📁 저장 위치: icons/bithumb/`);
...
async function main() {
  try {
    console.log("🚀 빗썸 아이콘 다운로더 시작...\n");

    // 1. 티커 목록 읽기
    const tickers = readTickersFromJson("ticker/bithumb-tickers.json");
