...
// 바이낸스 아이콘 URL 패턴
const BINANCE_ICON_URL_PATTERN =
  "https://bin.bnbstatic.com/static/assets/logos/{ticker}.png";
...
function downloadAndResizeIcon(ticker) {
  return new Promise((resolve, reject) => {
    const url = BINANCE_ICON_URL_PATTERN.replace("{ticker}", ticker);
    const dest = `icons/binance/${ticker}.webp`;
...
async function downloadAllIcons(tickers, delay = 100) {
  ...
  console.log(`📁 저장 위치: icons/binance/`);
...
async function main() {
  try {
    console.log("🚀 바이낸스 아이콘 다운로더 시작...\n");

    // 1. 티커 목록 읽기
    const tickers = readTickersFromJson("ticker/binance-tickers.json");
