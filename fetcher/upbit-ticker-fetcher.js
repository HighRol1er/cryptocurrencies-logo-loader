...
// 업비트 API 엔드포인트
const UPBIT_API_URL = "https://api.upbit.com/v1/market/all";
...
function fetchUpbitTickers() {
  return new Promise((resolve, reject) => {
    console.log("📥 업비트 API에서 거래 목록 가져오는 중...");

    https
      .get(UPBIT_API_URL, options, (res) => {
...
async function main() {
  try {
    console.log("🚀 업비트 거래 목록 수집 시작...\n");

    // 1. API에서 데이터 가져오기
    const rawTickers = await fetchUpbitTickers();

    // 2. 데이터 처리
    const processedTickers = processTickers(rawTickers);

    // 3. JSON 파일로 저장 (ticker 폴더에)
    const filename = "ticker/upbit-tickers.json";
